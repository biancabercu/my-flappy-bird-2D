...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vector<unsigned short> indices =
		{
			...
			0, 2, 3,	// indices for third triangle
			0, 1, 2		// indices for fourth triangle
		};


		vector<VertexFormat> verticesSquare
		{
			VertexFormat(glm::vec3(5, 5,  5), glm::vec3(0, 1, 1)),
			VertexFormat(glm::vec3(5, 9, 5), glm::vec3(0, 1, 1)),
			VertexFormat(glm::vec3(9, 9, 5), glm::vec3(0,1,1)),
			VertexFormat(glm::vec3(9, 5, 5), glm::vec3(0, 1, 1)),
		};

		vector<unsigned short> indicesSquare =
		{
		...
		meshes["square"]->InitFromData(verticesSquare, indicesSquare);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(-2.5f, 1.0f, 0), glm::vec3(0.25f));

	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-3.5f, 2.0f, 0), glm::vec3(0.25f));
